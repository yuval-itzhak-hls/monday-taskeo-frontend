...
const promptForModeIfNotProvided = async (mode) => {
    if (!mode) {
        mode = await PromptService.promptSelectionWithAutoComplete('Select app environment variables management mode', Object.values(APP_VARIABLE_MANAGEMENT_MODES));
    }
    return mode;
};
...
const promptForValueIfNotProvided = async (mode, value) => {
    if (!value && isValueRequired(mode)) {
        value = await PromptService.promptForHiddenInput('value', 'Enter value for environment variable', 'You must enter a value');
    }
    return value;
};
...
export default class Env extends AuthenticatedCommand {
    ...
    static flags = Env.serializeFlags(addRegionToFlags({
        appId: Flags.integer({
            char: 'i',
            aliases: ['a'],
