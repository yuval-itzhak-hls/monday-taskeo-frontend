...
export const mondaycodercSchema = z
    ...
    RUNTIME: z
        ...
        errorMap: () => ({
            message: 'Invalid Runtime in .mondaycoderc. Supported runtimes are Python, Java, Go, PHP, Ruby, Node.js, NETCore',
        }),
    })
        .optional(),
    ...
    if (data.RUNTIME_VERSION) {
        if (data.RUNTIME === 'Python') {
            if (!/^3\.(10|11|12)\.\d+$/.test(data.RUNTIME_VERSION || '')) {
                throw new Error('Invalid RUNTIME_VERSION for Python in .mondaycoderc. Allowed versions are 3.10.x, 3.11.x, 3.12.x');
            }
            return true;
        }
        ...
        if (data.RUNTIME === 'Ruby') {
            if (!/^3\.([1-3])\.\d+$/.test(data.RUNTIME_VERSION || '')) {
                throw new Error('Invalid RUNTIME_VERSION for Ruby in .mondaycoderc. Allowed versions are 3.1.x, 3.2.x and 3.3.x');
            }
            return true;
        }
        if (data.RUNTIME === 'Node.js') {
            ...
            if (!/^(18|20|22)\.\d+\.\d+$/.test(data.RUNTIME_VERSION || '')) {
                throw new Error('Invalid RUNTIME_VERSION for Node.js  in .mondaycoderc. Allowed versions are 18.x.x, 20.x.x, 22.x.x');
            }
            return true;
        }
