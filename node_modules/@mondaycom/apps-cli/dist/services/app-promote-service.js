import { DEFAULT_DELAY_POLLING_MS } from '../consts/app-promote.js';
import { APP_VERSION_STATUS } from '../consts/app-versions.js';
import { promoteAppUrl, pullPromoteStatusUrl } from '../consts/urls.js';
import { execute } from './api-service.js';
import { HttpMethodTypes } from '../types/services/api-service.js';
import logger from '../utils/logger.js';
import { TIME_IN_MILLISECONDS, sleep } from '../utils/time-utils.js';
...
export const shouldPromoteLatestDraftVersion = async () => {
    ...
    const userChoice = await PromptService.promptList('How would you like to promote the app?', [PROMOTE_LATEST_DRAFT_VERSION, CHOOSE_VERSION], PROMOTE_LATEST_DRAFT_VERSION);
    return userChoice === PROMOTE_LATEST_DRAFT_VERSION;
};
export const promoteAppTask = async (ctx) => {
    ...
    if (!location) {
        logger.error('The promotion was triggered, ' +
...
const fetchPromotionStatus = async (urlToPull) => {
    return execute({
        url: urlToPull,
        headers: { Accept: 'application/json' },
...
const handleStatusUpdate = (task, status) => {
    switch (status) {
...
const pollPromotionStatus = async (urlToPull, task) => {
...
export const pullPromoteStatusTask = async (ctx, task) => {
